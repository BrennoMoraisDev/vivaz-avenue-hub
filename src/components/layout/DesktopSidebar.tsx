...
const DesktopSidebar = ({ items }: DesktopSidebarProps) => {
    ...
    <aside className="hidden md:flex md:w-72 md:flex-col md:fixed md:inset-y-0 border-r border-border bg-card">
      ...
      <div className="border-t border-border p-3">
        <button
          ...
          className="flex w-full items-center gap-3 rounded-xl px-4 py-3 text-sm text-muted-foreground transition-all hover:bg-surface-hover hover:text-foreground"
        >
          <User size={18} />
          <span>Perfil</span>
        </button>
        <button
          onClick={() => signOut()}
          className="flex w-full items-center gap-3 rounded-xl px-4 py-3 text-sm text-muted-foreground transition-all hover:bg-destructive/10 hover:text-destructive"
        >
          <LogOut size={18} />
          <span>Sair</span>
        </button>
