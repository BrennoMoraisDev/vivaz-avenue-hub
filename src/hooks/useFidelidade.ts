...
export function useFidelidade() {
  ...
  const fetchFidelidade = useCallback(async () => {
    ...
    try {
      // Busca configurações (meta e se está ativo)
      const { data: configData } = await supabase
        .from('configuracoes')
        .select('chave, valor')
        .in('chave', ['fidelidade_pontos_para_resgate', 'fidelidade_ativo']) as { data: any[] | null };

      const metaConfig = configData?.find((c: any) => c.chave === 'fidelidade_pontos_para_resgate');
      const ativoConfig = configData?.find((c: any) => c.chave === 'fidelidade_ativo');

      const meta = metaConfig?.valor ? parseInt(metaConfig.valor) : META_PADRAO;
      const isAtivo = ativoConfig ? ativoConfig.valor === 'true' : true;
      setAtivo(isAtivo);
