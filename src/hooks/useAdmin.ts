...
export function useBarbeiros() {
  ...
  const atualizar = async (id: string, b: Partial<Barbeiro>) => {
    // Remove fields that shouldn't be updated directly or are virtual
    const { id: _, created_at: __, ...updateData } = b;
    const { error } = await (supabase.from('barbeiros') as any).update(updateData).eq('id', id);
    if (!error) await fetch();
    return { error };
  };
