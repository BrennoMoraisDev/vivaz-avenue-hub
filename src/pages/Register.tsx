...
import { Mail, Lock, User, Phone, Loader2, Crown } from 'lucide-react';
...
const Register = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    const { error } = await signUp(email, password, nome, telefone || undefined);
    if (error) {
  ...
  const inputClass = "w-full rounded-xl border border-border bg-secondary py-3 pl-10 pr-4 text-sm text-foreground placeholder:text-muted-foreground transition-all focus:border-primary focus:outline-none focus:ring-1 focus:ring-primary";
    ...
    <div className="flex min-h-screen items-center justify-center px-4 py-8">
      <div className="w-full max-w-md animate-fade-in">
        ...
        <div className="glass rounded-2xl p-8">
          <h2 className="mb-6 text-center font-heading text-xl font-semibold text-foreground">Cadastro</h2>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <div className="relative">
                ...
                <input type="text" placeholder="Seu nome" value={nome} onChange={(e) => setNome(e.target.value)} className={inputClass} required />
              </div>
            </div>
            <div>
            ...
            <div>
              <label className="mb-1.5 block text-xs font-medium text-muted-foreground">Telefone <span className="text-muted-foreground/60">(opcional)</span></label>
              <div className="relative">
                <Phone size={16} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-muted-foreground" />
                <input type="tel" placeholder="(00) 00000-0000" value={telefone} onChange={(e) => setTelefone(e.target.value)} className={inputClass} />
              </div>
            </div>
            <div>
            ...
            <div>
              ...
              <div className="relative">
                <Lock size={16} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-muted-foreground" />
                <input type="password" placeholder="Repita a senha" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} className={inputClass} required />
              </div>
            </div>
